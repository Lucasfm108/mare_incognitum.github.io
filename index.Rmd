...
Os equilíbrios do jogo serão diferentes a depender do tipo de vacância (para simplifcar, vamos supor que $p<j_2^1$):

<div style="float: left; width: 40%;">

**Vacâncias Proximais**:

Quando a vacância for proximal, o Presidente poderá escolher à vontade o seu nomeado, uma vez que o novo mediano será igual ao antigo. Logo, em nenhuma hipótese o Senado irá rejeitar sua escolha, isto é, $n=p$.
...
Quando a vacância for distal, o ponto ideal do nomeado será uma função da localização do Presidente, do Senado, do antigo mediano e do novo:

<p style="border:3px; border-style:dashed; border-color:#3D9970; padding: 1em;font-size:25px;">
$$ n(p,s,q,j_2^1) =
  \begin{cases}
    q  & se \quad s \geq q\\
    I_s  & se \quad s\in(\frac{(q+j_2^1)}{2}, q)\\
    p  & se \quad s \leq \frac{(q+j_2^1)}{2}\\
  \end{cases} $$
</p>
...
As conclusões da MTM podem ser testadas de diversas formas. Nos deteremos sobre duas delas.
